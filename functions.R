# support function to deal with data type for ALE
predict_tidymodels <- function(X.model, newdata) {
  # X.model = model_final_xgb
    ...
    if (class(X[, J]) == "factor") {
      ...
      barplot(
        ...
        las = 3
             ...
             "integer") {
      z = c(min(X[, J]), as.numeric(quantile(X[, J][X[, J] != -999], seq(
        1 / K,
        1, length.out = K
      ), type = 1)))
      z = unique(z)
